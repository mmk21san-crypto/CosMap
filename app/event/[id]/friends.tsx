...
export default function EventFriendsScreen() {
  const router = useRouter();
  const { id } = useLocalSearchParams<{ id: string }>();

  const eventId = Number(id);
  const event = ALL_EVENTS.find((e) => e.id === eventId);
  const friends = FOLLOWING.filter((u) => u.attending.includes(eventId));
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.page}>
        ...
        <Text style={styles.title}>イベント参加予定のフレンド</Text>
        <Text style={styles.sub}>{event ? event.name : ''}</Text>
...
const styles = StyleSheet.create({
  ...
  sub: { color: C.mid, fontSize: 12, fontFamily: 'Nunito_600SemiBold', marginBottom: 4 },
